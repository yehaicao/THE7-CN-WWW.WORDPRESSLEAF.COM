...
class WPBMap {
	...
	public static function map( $tag, $attributes ) {
		...
		if ( empty( $attributes['name'] ) ) {
			trigger_error( sprintf( __( "Wrong name for shortcode:%s. Name required", LANGUAGE_ZONE ), $tag ) );
		} elseif ( empty( $attributes['base'] ) ) {
			trigger_error( sprintf( __( "Wrong base for shortcode:%s. Base required", LANGUAGE_ZONE ), $tag ) );
		} else {
	...
	public static function getParam( $tag, $param_name ) {
		if ( ! isset( self::$sc[$tag] ) )
			return trigger_error( sprintf( __( "Wrong name for shortcode:%s. Name required", LANGUAGE_ZONE ), $tag ) );
	...
	public static function addParam( $name, $attribute = Array() ) {
		...
		if ( ! isset( self::$sc[$name] ) )
			return trigger_error( sprintf( __( "Wrong name for shortcode:%s. Name required", LANGUAGE_ZONE ), $name ) );
		elseif ( ! isset( $attribute['param_name'] ) ) {
			trigger_error( sprintf( __( "Wrong attribute for '%s' shortcode. Attribute 'param_name' required", LANGUAGE_ZONE ), $name ) );
		} else {
	...
	public static function modify( $name, $setting_name, $value = '' ) {
		...
		elseif ( $setting_name === 'base' ) {
			return trigger_error( sprintf( __( "Wrong setting_name for shortcode:%s. Base can't be modified.", LANGUAGE_ZONE ), $name ) );
		}
